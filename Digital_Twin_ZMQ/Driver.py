...
class CombinedController:
    ...
    def wfc_controller(self, id, current_time, measurements):
        ...
        if self.Activate_Prediction_Model:
    ...
    def prediction_model(self, current_time, measurements):
        ...
        FOWT_pred_state = 'BlPitchCMeas'                                # Choose FOWT state used for prediction outputs
        MLSTM_MODEL_NAME = 'StdParams_14400'                            # Define trained MLSTM-model name
        FUTURE_WAVE_FILE = f"WaveData_Load_Case_{self.Sea_State}.csv"   # Define incoming wave data file according to simulation
        ...
        with open(config_file_path, 'r') as file: config_data = yaml.safe_load(file)

        time_horizon = config_data['time_horizon']                      # Get time horizon from trained model
        ...
        # Data for website monitoring
        RotSpeed    = measurements["RotSpeed"] * 60 / (2 * np.pi)   # Rotor speed (rad/s to rpm)
        WE_Vw       = measurements["WE_Vw"]                         # Estimated wind speed
        VS_GenPwr   = measurements["VS_GenPwr"]                     # Generated power
    ...
    def update_system_state(self, current_time, save_to_csv=False, csv_file_path=None):
        if current_time - self.last_data_check_time >= self.chunk_duration/10:
            ...
            if not new_data.empty: 
                ...
                self.stress_history.process_data()
